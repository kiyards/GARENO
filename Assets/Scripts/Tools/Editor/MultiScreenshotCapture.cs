...
namespace MultiScreenshotCaptureNamespace
{
    ...
    public class MultiScreenshotCapture : EditorWindow
    {
        ...
        // ===== Hotkey-friendly entry point =====
        private void TriggerCapture()
        {
            // Guard for invalid state (also used by menu/shortcut)
            if (queuedScreenshots.Count != 0 || resolutionMultiplier <= 0f)
            {
                Debug.LogWarning("MultiScreenshotCapture: capture is already in progress or resolutionMultiplier <= 0");
                return;
            }
        ...
        // ===== MenuItem hotkey (classic) =====
        [MenuItem("Window/Multi Screenshot Capture/Capture Screenshots _F12")] // F12
        private static void CaptureFromMenu()
        {
            var window = GetWindow<MultiScreenshotCapture>();
            window.TriggerCapture();
        }

        // ===== Shortcut Manager (Unity 2019.1+) =====
#if UNITY_2019_1_OR_NEWER
        [Shortcut("Multi Screenshot Capture/Capture", KeyCode.F12)]
        private static void CaptureShortcut()
        {
            var window = GetWindow<MultiScreenshotCapture>();
            window.TriggerCapture();
        }
